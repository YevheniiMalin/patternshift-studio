...
export default function Home() {
  ...
  useEffect(() => {
    const generatedPattern = window.sessionStorage.getItem("patternshift-generated-pattern");
    if (!generatedPattern) return;
    const generatedName = window.sessionStorage.getItem("patternshift-generated-name") || "PatternShift-preliminary-pattern.svg";
    window.sessionStorage.removeItem("patternshift-generated-pattern");
    window.sessionStorage.removeItem("patternshift-generated-name");
    void loadPatternFile(new File([generatedPattern], generatedName, { type: "image/svg+xml" }));
  }, []);
      ...
      <header className="atelier-header border-b border-[#d9d0c3]">
        <div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-7">
          ...
          <div className="flex items-center gap-2">
            <Button asChild variant="ghost" size="sm" className="rounded-full text-[#61406b]">
              <Link href="/"><ArrowLeft /><span className="hidden sm:inline">{t.backToModes}</span></Link>
            </Button>
