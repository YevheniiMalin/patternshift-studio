...
const en = {
  ...
  intro: "Choose the starting point that matches what you have now. Each mode guides you one step at a time; local processing stays in your browser.",
  ...
  createBadge: "Local + optional SewFormer",
  createTitle: "Create a pattern from images",
  createDesc: "Use free local contour analysis, or connect your own SewFormer GPU server to reconstruct predicted panels and stitch relationships from a photo.",
  createAction: "Create from images",
  createFeatures: ["Free local pixel-contour workflow", "Optional SewFormer panels and stitch topology", "Measured SVG with construction checks"],
  warningTitle: "A useful draft, not an invisible guess",
  warning: "Local and AI results are preliminary. Hidden seams, production allowances and physical fit still require confirmation before fabric is cut.",
  privateTitle: "Private by design",
  privateDesc: "Local modes process files in your browser. SewFormer mode uploads the selected front image only to the server address you enter.",
...
const copy: Record<Language, HomeCopy> = {
  ...
  ru: {
    ...
    intro: "Выберите исходный материал, который у вас уже есть. Каждый режим проведёт вас по шагам; локальная обработка выполняется в браузере.",
    ...
    createBadge: "Локально + дополнительный SewFormer",
    createTitle: "Создать выкройку по изображениям",
    createDesc: "Используйте бесплатный локальный анализ контура или подключите собственный GPU-сервер SewFormer для реконструкции деталей и связей швов по фотографии.",
    createAction: "Создать по изображениям",
    createFeatures: ["Бесплатный локальный анализ контура", "Дополнительные детали и топология швов SewFormer", "Измеряемый SVG с проверкой конструкции"],
    warningTitle: "Полезный эскиз без скрытых догадок",
    warning: "Локальный и AI-результат являются предварительными. Скрытые швы, производственные припуски и посадку необходимо проверить до раскроя.",
    privateTitle: "Конфиденциальность по умолчанию",
    privateDesc: "В локальных режимах файлы обрабатываются в браузере. SewFormer отправляет выбранное фото спереди только на указанный вами сервер.",
    ...
    intro: "Valitse lähtökohta sen mukaan, mitä sinulla on nyt. Molemmat tilat ohjaavat vaiheittain; paikallinen käsittely pysyy selaimessa.",
    ...
    createBadge: "Paikallinen + valinnainen SewFormer",
    createTitle: "Luo kaava kuvista",
    createDesc: "Käytä ilmaista paikallista ääriviiva-analyysiä tai liitä oma SewFormer-GPU-palvelin kappaleiden ja saumasuhteiden ennustamiseen kuvasta.",
    createAction: "Luo kuvista",
    createFeatures: ["Ilmainen paikallinen ääriviivatyönkulku", "Valinnaiset SewFormer-kappaleet ja saumatopologia", "Mitattu SVG rakennetarkistuksineen"],
    warningTitle: "Hyödyllinen luonnos ilman piilotettuja arvauksia",
    warning: "Paikalliset ja AI-tulokset ovat alustavia. Piilosaumat, tuotannon saumanvarat ja fyysinen istuvuus on tarkistettava ennen leikkaamista.",
    privateTitle: "Yksityisyys oletuksena",
    privateDesc: "Paikallistilat käsittelevät tiedostot selaimessa. SewFormer lähettää valitun etukuvan vain antamaasi palvelinosoitteeseen.",
